...
function seedTextToNumber(seed) {
  // We will now convert the text seed to a number so "hello" will always generate the same map
  // cast to string
  var str = seed + "";
  var hash = 0;
...
function generateMap(seed, size, octaves, persistence, lacunarity, amplitude, frequency, type = "simplex") {
  const noise = createNoise();
  seed = seedTextToNumber(seed);
  noise.seed(seed);
  const width = size[0];
  const height = size[1];

  // We will now generate the map
  const map = new Array(width);
  for (let i = 0; i < width; i++) {
    map[i] = new Array(height);
  }

  // We will now generate the maps for each octave
  const octaveMaps = new Array(octaves);
  for (let i = 0; i < octaves; i++) {
    octaveMaps[i] = new Array(width);
    for (let j = 0; j < width; j++) {
      octaveMaps[i][j] = new Array(height);
    }
  }

  // We will now generate the octave maps
  for (let i = 0; i < octaves; i++) {
